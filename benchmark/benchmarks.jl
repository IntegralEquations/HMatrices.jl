...
for (name, K, p) in kernels
    ...
    # bench assemble
    SUITE[name]["assemble cpu"] = @benchmarkable assemble_hmatrix(
        $K,
        $Xclt,
        $Xclt;
        adm = $adm,
    ...
    SUITE[name]["assemble threads"] = @benchmarkable assemble_hmatrix(
        $K,
        $Xclt,
        $Xclt;
        adm = $adm,
    ...
    if p
        ...
        SUITE[name]["gemv cpu"] =
            @benchmarkable mul!($y, $H, $x, $1, $0; threads = false, global_index = $p)
        SUITE[name]["gemv threads"] =
            @benchmarkable mul!($y, $H, $x, $1, $0; threads = true, global_index = $p)
