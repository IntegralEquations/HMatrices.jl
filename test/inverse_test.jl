...
@elapsed H = HMatrix(K,Xclt,Yclt;threads=false,distributed=false)

H_full      = Matrix(H)
H_full_inv  = inv(H_full)

posthook = HMatrices.MulPostHook(TSVD(;atol=1e-10))
Hinv = inv(H,posthook)

Hinv_full = Matrix(Hinv)

@show norm(Hinv_full - H_full_inv)
