...
function getcol!(col, L::MulLinearOp, j)
    ...
    R = L.R
    if !isnothing(R)
        get_block!(col, R, :, j, true)
    end
    P = L.P
    if !isnothing(P)
        get_block!(col, P, :, j, true)
    end
    return col
end

get_block!(out, L::MulLinearOp, :, j::Int) = getcol!(out, L, j)
...
function getcol!(col, adjL::Adjoint{<:Any,<:MulLinearOp}, j)
    ...
    R = L.R
    if !isnothing(R)
        get_block!(col, adjoint(R), :, j, true)
    end
    P = L.P
    if !isnothing(P)
        get_block!(col, adjoint(P), :, j, true)
    end
    return col
end

get_block!(out, L::Adjoint{<:Any,<:MulLinearOp}, :, j::Int) = getcol!(out, L, j)
